...
【任务】
1. **分析思考**：首先思考文章各章节的主题，然后逐一审视图片描述，找到最匹配的图片。
2. **选择图片**：选择最多 5 张相关性最强的图片。注意区分不同统计图的具体含义（如“成本”vs“出版量”），不要混淆。
3. **精准定位**：核心是找到文中与图片最相关的**具体句子**。请提取该处原文（约10-20字）作为 `insert_after_text`。图片将被插入到这段文字之后。`after_heading` 仅用于辅助定位所在章节。

【输出格式】
先输出简短的思考过程（可选），然后输出 JSON 代码块：
...
{{
  "placements": [
    {{
      "image_id": "pic_4",
      "after_heading": "## 引言", 
      "insert_after_text": "Transformer的自注意力机制...",  // 必须是原文中存在的唯一片段，不要自己编造
      "caption": "图片说明"
    }}
  ]
}}
...
⚠️ **重要**：
- **image_id 必须精准**：必须使用图片列表中方括号内的 ID（如 pic_4）。
- **定位即上下文**：不要把图片简单地堆在标题下面，务必让它出现在描述相关概念的文字旁边。
"""
