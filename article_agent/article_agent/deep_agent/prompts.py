...
## 输入
- `article_id`: 文章 ID，用于读取素材和大纲

## 研究方法
1. 读取已分块的素材文本 (chunks.jsonl)
2. 根据大纲中每个章节的 keywords 和 required_evidence 提取：
   - **bullet_points**: 核心观点列表
   - **evidence**: 证据链（包含 claim, refs, quote）
3. 图片已通过 Hungarian 算法预分配，无需手动选择
...
## 输入
- `article_id`: 文章 ID，用于读取大纲和研究笔记

## 写作原则
1. **遵循大纲**: 按 outline.json 中的章节顺序撰写
...
## 任务
1. 读取所有章节草稿 (drafts/section_sec_N.md)
2. 合并为完整文章
3. 添加文章元信息（标题、日期）
4. 清理格式（去除多余空行）
5. 保存到 article/article.md
...
⚠️ **关键**: `article_content` 字段必须包含完整文章，Middleware 会自动填充如果内容过长被截断。
""".strip()
