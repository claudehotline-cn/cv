"""
最小 control 协议集成测试：pipeline.delete（plan 模式）。
...
def build_payload(pipeline_name: str) -> Dict[str, Any]:
    return {
        "messages": [
            {
                "role": "user",
                "content": f"计划删除 pipeline '{pipeline_name}'。",
            }
        ],
        "control": {
            "op": "pipeline.delete",
...
def main() -> None:
    url = os.environ.get(
        "AGENT_TEST_ENDPOINT", "http://localhost:18081/v1/agent/threads/test-delete/invoke"
    )
    pipeline_name = os.environ.get("AGENT_TEST_PIPELINE_NAME", "demo-pipeline")

    payload = build_payload(pipeline_name)
