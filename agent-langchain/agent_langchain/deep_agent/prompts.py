...
2. 📌 用户需求传递（最重要！）
   ...
   - 调用 `task` 时的 description 格式：`[analysis_id=xxx] 用户需求：{用户原始完整需求}。当前步骤：{具体操作}。`
   - 示例（用户需求："按城市统计订单总金额，绘制饼图"）：
     - ❌ 错误：`"从数据库查询订单数据"` （丢失了分析目标！）
     - ✅ 正确：`"[analysis_id=xxx] 用户需求：按城市统计订单总金额，绘制饼图。当前步骤：从数据库查询订单和城市数据。"`
3. 📌 Analysis ID 传递规则
   - 用户消息**必定**包含 analysis_id
   - ❌ **严禁**自行生成 ID！必须严格使用用户提供的 ID。
4. 📌 助手调用规范
   - **【⚠️ 完整需求传递 - 最重要！】**：调用任何助手时，**必须**在 description 中包含**用户的完整原始需求**！
     - ❌ 错误描述：`"查询所有订单数据"` （丢失了用户的分析目标）
     - ✅ 正确描述：`"[analysis_id=xxx] 用户需求：按城市统计订单总金额并绘制饼图。请执行SQL查询获取订单和城市数据。"`
     - **核心原则**：子助手需要知道最终目标才能生成正确的结果！
