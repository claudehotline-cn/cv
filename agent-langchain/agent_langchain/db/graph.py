...
def _build_db_graph() -> Any:
    ...
    def sql_agent_node(state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 为避免将 SQLDatabase/engine 放入 state（无法被 checkpoint 序列化），
        # 每次在节点内部根据 db_name 构造 SQLDatabase。
        try:
            sql_db = get_sql_database(db_name)
        except Exception as exc:
            _LOGGER.error("db.graph.sql_agent_node.sql_db_failed db_name=%s error=%s", db_name, exc)
            raise
