...
- 配置目录：统一从 `video-analyzer/config/graphs/` 读取图配置文件（后缀 `.yaml|.yml`）。
- 命名与选择：文件名（不含后缀）作为 `graph_id`，例如 `detector.yaml` → `graph_id=detector`。
- 启用与选择方式：
  - 配置方式：`engine.options.use_multistage: "true"`，`engine.options.graph_id: <name>`。
  - 覆盖方式（可选）：依然支持绝对路径 `engine.options.multistage_yaml` 用于临时测试，但生产建议优先用 `graph_id`。
- 热切换：
  - 入口：沿用现有 `/api/engine/set`（或新增 `/api/graph/set`），传入 `graph_id`；服务端在后台加载→校验→finalize 成功后，原子替换运行中的 Graph；失败则回滚上一份可用图，并返回详细错误。
  - 列表：新增接口列出 `config/graphs` 下的可用图清单（文件名、最近修改时间、校验状态）。
  - 校验：加载时做拓扑、节点依赖键、必需参数、可选资源（例如模型路径）校验，日志包含节点/边具体指向，便于快速排障。
- 运行态约束：切换图不重启媒体流、编码器、传输；仅重建分析链。复用通用资源（池、renderer、推理引擎等）可作为后续优化项。

## 输入源管理与兼容（补充）

- 兼容两种来源：
  - 静态配置：`app.yaml` 中的 `sources`（当前已有能力）；
  - 动态管理：保留通过前端/REST 在运行时添加/切换 RTSP 源（例如 `video-source-manager` 或已有 API）。
- 默认策略：`sources` 为可选项，不强制要求；若未配置，则沿用既有的“运行时添加/默认回放源”的行为，不改变你之前“无需配置 URL”即可观看的使用方式。
- 优先级（建议）：运行时动态添加/切换的源优先于静态配置；重启后回落到静态配置或空列表。
- 文档说明：在用户指南中明确“静态/动态”两种方式都受支持，动态方式可不修改 `app.yaml`；静态方式适合生产固定部署。
