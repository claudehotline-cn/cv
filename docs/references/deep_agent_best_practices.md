...
*   **Human-in-the-loop (通用机制)**: 在任意关键节点（如 SQL 执行前、多次重试失败后）使用 `interrupt_before` 暂停，赋予用户确认或修改状态的权限。

### 3. System Hints (系统旁白/导航)
...
## 8. Python 沙箱执行 (Python Sandbox Execution)
...
## 11. 全栈数据流最佳实践 (Full-Stack Data Flow)
...
## 12. LangChain 核心消息对象详解 (LangChain Message Objects)
