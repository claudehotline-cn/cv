...
- 订阅与 LRO 设计：`docs/design/subscription_pipeline/lro_subscription_design.md`
- 存储访问：`docs/design/storage/storage_详细设计.md`
...
- 关键实现与设计文档：
  - 推理与引擎：`tensorrt_engine.md`、`triton_inprocess_integration.md`
  - 多阶段 Graph：`multistage_graph_详细设计.md`
  - LRO 订阅执行：`lro_subscription_design.md`
  - 媒体传输：`webrtc-protocol.md`
  - 日志与指标：`LOGGING.md`、`METRICS.md`、`observability_详细设计.md`
...
- 设计文档：`controlplane_design.md`、`控制平面HTTP与gRPC接口说明.md`、`控制面错误码与语义.md`
...
- 核心职责：
  ...
  - 在 AnalysisPanel 页面中：
    ...
    - 提供分析开关、引擎/模型切换等操作按钮。
- 设计文档：`web_front_详细设计.md`、`observability_详细设计.md`
...
- 设计文档：`cv_训练流水线（training_pipeline）详细设计_v_1.md`（附录包含 MLflow 与 MinIO S3 仓库设计）

### 4.6 数据库与缓存

- MySQL：
  - 数据库 `cv_cp` 保存源、会话、图、模型、事件、日志及训练记录等实体。
  - 表结构概览与关系见 `storage_详细设计.md` 第 2 章。
...
详细表结构与索引策略见 `docs/design/storage/storage_详细设计.md` 第 2 章，本节仅给出关键实体的抽象：
...
- 通过多阶段 Graph、Batch/IOBinding、Triton In-Process 等手段降低 CPU/GPU 开销，详见 `zero_copy_execution_详细设计.md` 第 6 章。
...
- 数据库与训练：
  - 按 `storage_详细设计.md` 中的索引与归档策略规划容量与备份。
...
- 控制平面与订阅：
  ...
  - `docs/design/subscription_pipeline/subscription_pipeline_详细设计.md`
  - `docs/design/protocol/控制面错误码与语义.md`
  - `docs/design/protocol/控制平面HTTP与gRPC接口说明.md`
- 推理与多阶段图：
  - `docs/design/subscription_pipeline/tensorrt_engine.md`
  - `docs/design/subscription_pipeline/triton_inprocess_integration.md`
  - `docs/design/subscription_pipeline/multistage_graph_详细设计.md`
  - `docs/design/subscription_pipeline/zero_copy_execution_详细设计.md`
- 媒体与前端集成：
  - `docs/design/protocol/webrtc-protocol.md`
  - `docs/design/architecture/web_front_详细设计.md`
