    ...
    def on_llm_end(self, response: LLMResult, **kwargs: Any) -> Any:
        ...
        try:
            ...
            if response.llm_output and "token_usage" in response.llm_output:
                ...
                _LOGGER.info(
                    "[TokenCost] Run Cost: $%.6f | Tokens: %s (P:%s, C:%s) | Total Session: $%.4f",
                    cost,
                    total,
                    prompt,
                    completion,
            ...
            _LOGGER.warning("[TokenCost] Failed to calculate cost: %s", exc)
...
def setup_opentelemetry(service_name: str, otlp_endpoint: str = "http://jaeger:4317") -> bool:
    """
    ...
    try:
        ...
        from openinference.instrumentation.langchain import LangChainInstrumentor  # pyright: ignore[reportMissingImports]

        LangChainInstrumentor().instrument()

        _LOGGER.info("[Observability] OTel instrumented for %s -> %s", service_name, otlp_endpoint)
        return True
    except ImportError:
        _LOGGER.warning("[Observability] opentelemetry libs not found. Skipping instrumentation.")
        return False
    except Exception as exc:
        _LOGGER.error("[Observability] Failed to setup OTel: %s", exc)
        return False
