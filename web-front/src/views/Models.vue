    ...
    <template #header>
      <div class="card-header">
        ...
        <el-space>
          ...
          <el-button size="small" type="danger" @click="confirmRemoveById" :disabled="!modelId">Delete</el-button>
      ...
      <el-table-column label="操作" width="460" fixed="right">
        <template #default="{ row }">
          <el-space>
            ...
            <el-button size="small" type="danger" text @click="confirmRemove(row.id)" :disabled="!isRepoMode">删除</el-button>
          </el-space>
        </template>
      </el-table-column>
    </el-table>
          ...
          <div style="display:flex; flex-direction:column; gap:6px;">
            ...
            <div v-if="addForm.platform==='tensorrt_plan' && addIsOnnx">
              ...
              <div style="margin-top:6px;">
                <el-button size="small" type="danger" :disabled="!convertInProgress" @click="cancelConvert">取消</el-button>
              </div>
...
const hasStartedConvert = ref(false)
const convertInProgress = ref(false)
// 进度柔性推进（当后端事件稀疏时，提供连续感知，不超过阶段上限）
let progressTimer: any = null
let progressSoftCap = 85 // running 阶段上限，uploading 阶段提升至 99
function startProgressTick(){
  if (progressTimer) return
  progressIndeterminate.value = false
  progressTimer = window.setInterval(() => {
    const cap = progressSoftCap
    if (progressPercent.value >= cap) return
    const inc = progressPercent.value < 50 ? 0.5 : (progressPercent.value < 80 ? 0.3 : 0.1)
    // 保留 0.1 精度，UI 会四舍五入显示
    const next = Math.min(cap, Math.round((progressPercent.value + inc) * 10) / 10)
    progressPercent.value = next
  }, 500)
}
function updateSoftCap(cap: number){
  if (cap > progressSoftCap) progressSoftCap = cap
...
async function confirmRemoveById(){ if (!modelId.value) return; await confirmRemove(modelId.value) }
...
async function confirmRemove(id: string){
  try{
    if (!isRepoMode.value) { ElMessage.warning('仅仓库模式支持删除'); return }
    const ok = window.confirm(`确认删除模型 ${id} 及其文件？该操作不可恢复。`)
    if (!ok) return
    await cp.repoRemove(id)
    ElMessage.success('删除已提交')
    await load()
...
async function startConvertUpload(){
  ...
  try {
    ...
    currentJobId.value = (r?.data?.job || '') as string
    ...
    convertEs.addEventListener('state', (ev:any) => {
      try {
        ...
        if (typeof d.progress === 'number') {
          const pr = asPercent(d.progress)
          progressIndeterminate.value = false
          progressPercent.value = Math.max(progressPercent.value, pr)
        }
      } catch {}
      // 阶段性上限与基线
      if (convertPhase.value === 'uploading') {
        progressIndeterminate.value = false
        if (progressPercent.value < 90) progressPercent.value = 90
        updateSoftCap(99)
      } else if (convertPhase.value === 'running') {
        updateSoftCap(85)
      }
      if (convertPhase.value === 'failed') { progressIndeterminate.value = false; progressStatus.value = 'exception'; stopProgressTick() }
    })
...
async function cancelConvert(){
  try{
    ...
    await cp.repoConvertCancel(currentJobId.value)
    convertPhase.value = 'failed'
    progressIndeterminate.value = false
    progressStatus.value = 'exception'
    if (convertEs) { convertEs.close(); convertEs = null }
    stopProgressTick()
    convertInProgress.value = false
    ElMessage.success('已取消转换')
  } catch(e:any) {
    ElMessage.error(e?.message || '取消失败')
  }
}

// 自动触发：当用户先选文件后再切换平台或补填模型ID时，满足条件即开始转换/上传
watch([
  () => addFile.value,
  () => addForm.platform,
  () => addForm.model,
  ...
  if (!addDlg.value) return
  if (!addFile.value) return
  if (!addForm.model) return
  const isOnnx = isOnnxFile(addFile.value.name)
  if (addForm.platform === 'tensorrt_plan' && isOnnx) {
    if (!convertInProgress.value && !hasStartedConvert.value) { hasStartedConvert.value = true; startConvertUpload() }
  } else {
    if (!uploadInProgress.value && uploadStatus.value !== 'uploaded') startDirectUpload()
  }
})
